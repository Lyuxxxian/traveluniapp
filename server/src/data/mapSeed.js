/**
 * 地图种子数据（与 src/api/mapData.ts、src/api/map.ts fallback 对齐）
 * 坐标为景区范围内估算值，后续可在管理端精修。
 */
...
function seeds(categoryLabel, idStart, titles, tag) {
  return titles.map((title, index) => {
    ...
    return {
      ...
      desc: `${title}（坐标待精修，可在后续替换为真实经纬度）`,
      tags: tag ? [tag] : undefined,
    }
  })
}

function numberedTitles(prefix, count) {
...
export const MAP_ROUTES = [
  ...
  },
  {
    ...
    desc: '半间小食令、吉祥食集、彩虹小食令、灵山五关堂。',
  },
]

/** 与 map.ts detailExtras 一致 */
export const MAP_POINT_DETAILS = {
  101: {
...
function seedToPoint(category, seed) {
...
function buildMapPoints() {
  return Object.entries(MAP_POINT_SEEDS).flatMap(([category, items]) =>
    items.map((seed) => seedToPoint(category, seed)),
  )
}

export const mapCategoriesSeed = [...MAP_CATEGORIES]
export const mapPointsSeed = buildMapPoints()
export const mapRoutesSeed = [...MAP_ROUTES]
export const mapPointDetailsSeed = { ...MAP_POINT_DETAILS }
